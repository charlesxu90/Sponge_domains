...
=head1 Name
    ...
    del_domain_seq.pl --> This script is used to read domain info from iprscan result, and remove domain sequence for corresponding sequence in fasta file
...
=head1 Example
    ...
    perl  del_domain_seq.pl -d domain.tsv -f protein.fasta-o rmd_seq.fa
...
while(<DOMAIN>)
{
    chomp;
    @domain_info = split /\t/, $_;
    # 0 is name, 6 is start 7 is end
    if(!defined $DomainInfo{$domain_info[0]}{$domain_info[6]}){
        $DomainInfo{$domain_info[0]}{$domain_info[6]} = $domain_info[7];
    }elsif($DomainInfo{$domain_info[0]}{$domain_info[6]} == $domain_info[7]){
        # The domain info is duplicated
        next;
    }else{
        # Domains which start in the same location, take the longer one
        if ($DomainInfo{$domain_info[0]}{$domain_info[6]} < $domain_info[7]) {
            $DomainInfo{$domain_info[0]}{$domain_info[6]} = $domain_info[7];
        }
    }
}
...
#    next if $count >= 2; #For test
    ...
    for $sub_pos (@split_pos){
        ...
        my $seq_obj = Bio::Seq->new(-seq=>$new_seq,-display_id => $seq_id."$id_sufix",-alphabet => 'protein');
        print $OUT $seq_obj; 
    }
}
#close $PROTEIN;
